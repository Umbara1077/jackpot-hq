...
function renderLab() {
  ...
  const strats = G.digits ? ['quick', 'hot', 'cold', 'balanced'] : G.matrix?.pick === 1 ? ['quick'] : ['quick', 'smart', 'hot', 'cold', 'balanced', 'manual'];
  ...
  <div class="chip truth" style="margin-top:10px" id="truthchip">${STRATS[labStrat] && !G.digits ? esc(STRATS[labStrat].truth) : 'Every combination has identical odds — strategies are about fun and (for Smart Pick) not splitting a shared win.'}</div>
  ...
  <div id="labout"></div>`}`;
  $$('.gpick button', v).forEach(b => b.onclick = () => { labGame = b.dataset.g; labLines = []; manualSel = []; manualBonus = null; renderLab(); });
  $$('.strat', v).forEach(b => b.onclick = () => { labStrat = b.dataset.s; labLines = []; renderLab(); });
