...
function aiLogo(key) {
  ...
  // Super Intelligence isn't one maker's model, so it gets its own mark: four nodes
  // (one per provider) converging on a single point.
  const superTile = `<svg viewBox="0 0 40 40"><rect width="40" height="40" rx="10" fill="#0d0b1a"/>
    <defs><linearGradient id="superGrad" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="#ffd66b"/><stop offset="45%" stop-color="#ff8ad8"/><stop offset="100%" stop-color="#7ea8ff"/>
    </linearGradient></defs>
    <g stroke="url(#superGrad)" stroke-width="1.6" opacity=".85">
      <path d="M20 20 L10 10M20 20 L30 10M20 20 L10 30M20 20 L30 30"/>
    </g>
    <g fill="url(#superGrad)"><circle cx="10" cy="10" r="3"/><circle cx="30" cy="10" r="3"/><circle cx="10" cy="30" r="3"/><circle cx="30" cy="30" r="3"/></g>
    <circle cx="20" cy="20" r="4.6" fill="#0d0b1a" stroke="url(#superGrad)" stroke-width="2"/></svg>`;
  const M = {
    super: superTile,
...
const AI_MAKER = {
  super: 'Every maker', fable: 'Anthropic', opus: 'Anthropic', grok: 'xAI', sol: 'OpenAI', terra: 'OpenAI',
  geminipro: 'Google', geminiflash: 'Google', gemini2flash: 'Google', gemini: 'Google',
};
const AI_PROVIDERS = [
  { id: 'panel', name: 'Super Intelligence', brand: 'All makers', logo: 'super', models: ['super'] },
...
async function syncPull() {
  try {
    ...
    const j = await r.json().catch(() => ({}));
    if (!r.ok) { ACCT.lastSyncErr = j.detail || j.error || ('HTTP ' + r.status); return false; }
    ACCT.lastSyncErr = null;
    if (j.state && mergeRemote(j.state)) { save(); applyTheme(); refreshCurrentView(); }
    return true;
  } catch (e) { ACCT.lastSyncErr = String(e?.message || e); return false; }
}
function syncPushSoon(immediate) {
  if (!onWeb() || !ACCT.user || !ACCT.sync) return Promise.resolve({ ok: false, err: 'sync off' });
  clearTimeout(syncT);
  const run = async () => {
    try {
      ...
      const j = await r.json().catch(() => ({}));
      if (r.ok) { ACCT.lastPush = Date.now(); S.syncedAt = payload.pushedAt; localStorage.setItem('jhq', JSON.stringify(S)); return { ok: true }; }
      return { ok: false, err: j.detail || j.error || ('HTTP ' + r.status) };
    } catch (e) { return { ok: false, err: String(e?.message || e) }; }
  };
  // after login / Sync now: upload right away so existing tickets land in D1
  if (immediate) return run();
  syncT = setTimeout(() => { run(); }, 2500);
  return Promise.resolve({ ok: false, err: 'queued' });
}
function openAccount() {
  if (ACCT.user) {
    ...
    const sn = $('#syncNow'); if (sn) sn.onclick = async () => {
      await syncPull();
      const res = await syncPushSoon(true);
      toast(res.ok ? 'Synced to cloud' : ('Sync failed: ' + (res.err || 'unknown')), !!res.ok);
    };
...
// Needs two providers to exist at all, so it's the top pick when the site has them.
const AI_BLURB = {
...
async function aiProbe() {
  ...
  try {
    ...
    // Prefer the panel when the site can actually run one, then the cheap default.
    if (avail.length && !avail.includes(aiModel)) {
      aiModel = avail.includes('super') ? 'super' : avail.includes('geminiflash') ? 'geminiflash' : avail[0];
    }
...
function openAiModelPicker() {
  ...
  const groups = AI_PROVIDERS.map((p) => {
    const buttons = p.models.map((k) => {
      ...
      // raw text only — escaped once at render below, never pre-escaped here
      const sub = live && !m.available
        ? (k === 'super' ? 'Needs two AI providers configured' : 'Unavailable on this site')
        : (AI_BLURB[k] || `${p.brand} · ${p.name}`);
      return `<button type="button" data-ai="${k}" class="aimodel ${k === aiModel ? 'on' : ''}" ${live && !m.available ? 'disabled' : ''}>
        ${aiLogo(k)}
        <span class="aimeta"><b class="grad">${esc(m.name)}</b><small>${esc(sub)}</small></span>
      </button>`;
    }).join('');
...
async function aiGenerate() {
  ...
  // Budget comes from the chosen effort. The panel needs far more: each seat gets two
  // attempts (90s + 60s) and the chair the same, so the worst realistic case is ~240s.
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), panel ? Math.max(330000, E.ms + 60000) : E.ms);
  try {
    const r = await fetch(ep, {
      method: 'POST',
      headers: { 'content-type': 'application/json', ...(S.aiPass ? { 'x-app-pass': S.aiPass } : {}) },
    ...
    // Name the panel so a "Super Intelligence" verdict is auditable rather than a black box
    const who = Array.isArray(j.panel) && j.panel.length
